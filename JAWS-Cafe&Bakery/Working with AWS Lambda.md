...
In this lab, I deployed and configured a serverless computing solution using AWS Lambda. The Lambda function I worked with generates a daily sales analysis report. To do this, it retrieves data from a database and emails the results automatically.

The database connection details were securely stored in AWS Systems Manager Parameter Store, which allowed the Lambda function to access them without hard-coding credentials.

The database itself was hosted on an Amazon EC2 LAMP instance running:
...
This setup allowed me to practice integrating Lambda with external resources, securely managing configuration parameters, and automating data reporting using AWS services.
