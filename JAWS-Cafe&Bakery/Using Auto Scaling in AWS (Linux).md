...
In this lab, I used the AWS Command Line Interface (AWS CLI) to deploy a web server on an Amazon EC2 instance and then created an Amazon Machine Image (AMI) from that instance. The AMI was then used as the launch template for Amazon EC2 Auto Scaling, enabling the system to scale in and out automatically based on demand.

Additionally, I configured an Elastic Load Balancer (ELB) to distribute incoming traffic across multiple EC2 instances running in different Availability Zones. This ensured both load distribution and high availability across the environment.
...
By completing this lab, I implemented:

- A web server running on Linux

- Creation and use of a custom AMI

- Configuration of an Auto Scaling Group that launches instances across Availability Zones

- A Load Balancer distributing traffic across scalable EC2 instances

This system automatically adapts to variable workloads:

- Scaling out to add more instances when load increases

- Scaling in to reduce capacity and cost during periods of low demand
